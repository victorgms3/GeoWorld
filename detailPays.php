...
/*
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_wikipedia'])) {
    // Récupérer les données depuis Wikipedia
    $wikipediaData = getWikipediaData($pays["Name"]);

    if ($wikipediaData) {
        // Exemple : Mettre à jour les champs avec des données fictives extraites de Wikipedia
        $population = $wikipediaData['population'] ?? $pays["Population"]; // Remplacez par la bonne clé
        $pnb = $wikipediaData['gdp'] ?? $pays["GNP"]; // Remplacez par la bonne clé
        $chef = $wikipediaData['head_of_state'] ?? $pays["HeadOfState"]; // Remplacez par la bonne clé
        $esperance = $wikipediaData['life_expectancy'] ?? $pays["LifeExpectancy"]; // Remplacez par la bonne clé
        ...
        // Recharger les données mises à jour
        $pays = getDetailsPays($idPays);
        echo "<p style='color: green;'>Les informations ont été mises à jour avec succès depuis Wikipedia.</p>";
    } else {
        echo "<p style='color: red;'>Impossible de récupérer les données depuis Wikipedia.</p>";
    }
}
    */
